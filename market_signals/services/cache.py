...
# ── 데이터 갱신 ───────────────────────────────
def refresh_data(max_stocks: int = 300) -> dict:
    """
    한국투자증권 API에서 최신 데이터를 가져와
    4가지 카테고리로 분류 후 캐시에 저장합니다.
    """
    logger.info("데이터 갱신 시작...")
    raw_df  = fetch_all_markets(max_stocks_per_market=max_stocks)
...
def get_cached_data(max_stocks: int = 300) -> dict:
    """
    캐시된 데이터 반환
    ...
    if file_data:
        ...
        # 백그라운드에서 최신 데이터 갱신
        t = threading.Thread(target=refresh_data, args=(max_stocks,), daemon=True)
        t.start()
        return file_data

    # 캐시 없으면 동기 수집 (최초 1회)
    return refresh_data(max_stocks)


# ── 자동 스케줄러 ─────────────────────────────
def _scheduler_loop(max_stocks: int):
...
def start_scheduler(max_stocks: int = 300):
    """백그라운드 스케줄러 스레드 시작"""
    t = threading.Thread(
