...
def _df_to_records(df): return df.to_dict(orient="records")
...
def _scheduler_loop():
    while True:
        ...
        if now.weekday() < 5 and 9 <= now.hour < 16:
            try: refresh_data()
            except Exception as e: logger.error(f"스케줄러 오류: {e}")
            time.sleep(REFRESH_INTERVAL)
        else:
            time.sleep(3600)

def start_scheduler():
    threading.Thread(target=_scheduler_loop, daemon=True, name="Stock-Scheduler").start()
    logger.info("스케줄러 시작")