...
def get_market_dashboard_data(request):
    try:
        ...
        # 프론트엔드 UI(image_0a7067.png)에 필요한 모든 정보를 구조화합니다.
        dashboard_data = {
            "market_signals": {
            ...
            "indices": {
                "kospi": {
                    ...
                    "change_price": safe_get("kospi_diff", "0.00"), # 변동폭 (예시의 ▲/▼ 부분 대응)
                    "trend_status": "상승 추세", # 퀀트 로직에 따른 상태 (추후 자동화 가능)
                    "exposure_guide": "80-100%", # 권장 비중
                    "distribution_day": real_data.get("kospi_dist_day", 0), # 분산일
